...
def includeme(config):
    ...
    # frontend
    def preview_overwrite_param(request, elements, kwargs):
        """overwrite_theme 這個 query param 可以用來暫時覆蓋 request.current_theme ，用在預覽樣板用。
        所以一旦 url 上出現了 overwrite_theme 且其值不為空，就要全部自動的加在所有 route_url 後"""
        query = kwargs.setdefault('_query', {})
        if request.GET.get('overwrite_theme', None):
            query.setdefault('overwrite_theme', request.GET['overwrite_theme'])
        return elements, kwargs

    config.add_route('homepage', '/', pregenerator=preview_overwrite_param)
    config.add_route('news_list', '/news/list', pregenerator=preview_overwrite_param)
    config.add_route('news_get', r'/news/get/{news_id:\d+}', pregenerator=preview_overwrite_param)
    config.add_route('telext', '/telext', pregenerator=preview_overwrite_param)
    config.add_route('links', '/links', pregenerator=preview_overwrite_param)
    config.add_route('page_get', '/page/get/{page_id:\d+}', pregenerator=preview_overwrite_param)
    config.add_route('login', '/login', pregenerator=preview_overwrite_param)
    config.add_route('logout', '/logout', pregenerator=preview_overwrite_param)

    # backend
    config.add_route('backend_homepage', '/backend/', factory=auth_user_factory, pregenerator=preview_overwrite_param)

    config.add_route('backend_site_config_edit', '/backend/site/config/edit', factory=admin_factory,
                                                                              pregenerator=preview_overwrite_param)
    ...
    config.add_route('backend_theme_config_general_edit', '/backend/theme_config/general/edit/{theme_name}',
                     factory=admin_factory, pregenerator=preview_overwrite_param)
    ...
    config.add_route('backend_link_delete', '/backend/link/delete/{link_id:\d+}',
                     factory=link_edit_factory, pregenerator=preview_overwrite_param)
