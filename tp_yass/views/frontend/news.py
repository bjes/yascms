...
def news_list_view(request):
    news = DAL.get_news_list(category_id=request.GET.get('category_id', None),
                             quantity_per_page=int(request.sys_config['site_homepage_news_quantity']))
    return {'news': news,
            'page_quantity': _calculate_page_quantity(news, int(request.sys_config['site_homepage_news_quantity']))}
